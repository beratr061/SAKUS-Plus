...
Canlı araç takibi • Araç sorgulama • Sefer saatleri • Durak bilgileri • Hat güzergahları • Yakınımdaki Hatlar • Nasıl Giderim • Haberler & Duyurular
...
- [📡 API Dokümantasyonu](#-api-dokümantasyonu)
  ...
  - [Araç Sorgulama API](#8--araç-sorgulama-api)
  - [Durağa Yaklaşan Otobüsler](#9--durağa-yaklaşan-otobüsler)
  - [Yakınımdaki Hatlar API](#10--yakınımdaki-hatlar-api)
  - [Nasıl Giderim API](#11--nasıl-giderim-api)
...
| 📍 **Yakınımdaki Hatlar** | Cihaz konumuna göre 50m–1500m yarıçapta yakın hatları tarama, mesafeye göre sıralama, hat detayına hızlı geçiş |
| 🧭 **Nasıl Giderim** | Başlangıç ve varış noktası seçerek toplu taşıma güzergah önerisi alma (OTP tabanlı) |
...
| 🔎 **Araç Sorgulama** | Plaka veya kapı numarası ile canlı araç konumu sorgulama, SSE stream üzerinden gerçek zamanlı takip |
...
### 8. 🔎 Araç Sorgulama API

Plaka numarası veya kapı numarası (busNumber) ile araç konumu sorgulama. SSE stream üzerinden canlı takip yapılır.

#### 8a. SSE Stream ile Canlı Sorgulama
...
Uygulama SSE stream'e bağlanır ve gelen tüm araç verilerini `plate` ve `busNumber` alanlarına göre filtreler. Eşleşen araçlar gerçek zamanlı olarak ekranda gösterilir.
...
**Yanıt Modeli:** API'den gelen ham JSON:

| JSON Alanı | Tip | Açıklama |
|------------|-----|----------|
| `plate` | `String` | Araç plakası (örn: "54 J 0165") |
| `busNumber` | `Int` | Kapı numarası (örn: 19165) |
| `lineNumber` | `String` | Hat numarası |
| `lineName` | `String` | Hat adı |
| `status` | `String` | Araç durumu (AT_STOP, IN_TRAFFIC, MOVING, vb.) |
| `speed` | `Int` | Anlık hız (km/h) |
| `nextStopName` | `String` | Sonraki durak adı |
| `atStopName` | `String` | Mevcut durak adı |
| `distNextStopMeter` | `Double` | Sonraki durağa mesafe (metre) |
| `etaS` | `Double` | Tahmini varış süresi (saniye) |
| `routeName` | `String` | Güzergah adı |
| `startLocation` | `String` | Başlangıç durağı |
| `endLocation` | `String` | Bitiş durağı |
| `location.coordinates` | `[Double]` | [boylam, enlem] |

**Durum Değerleri (Türkçe çevirili):**

| API Değeri | Türkçe Karşılığı |
|------------|------------------|
| `AT_STOP` | Durakta |
| `IN_TRAFFIC` | Trafikte |
...
**Örnek İstek (REST fallback):**

```bash
curl -H "Origin: https://ulasim.sakarya.bel.tr" \
     "https://sbbpublicapi.sakarya.bel.tr/api/v1/VehicleTracking?AsisId=90"
```

---

### 9. 🚏 Durağa Yaklaşan Otobüsler
...
### 10. 📍 Yakınımdaki Hatlar API

Kullanıcının konumuna belirli bir yarıçap içindeki toplu taşıma hatlarını getirir.

**Endpoint:**

```http
GET /Ulasim/nearlines?latitude={lat}&longitude={lng}&radiusMeters={radius}&busType={busType}
```
...
| Parametre | Tip | Varsayılan | Açıklama |
|-----------|-----|------------|----------|
| `latitude` | `Double` | - | Kullanıcı enlemi |
| `longitude` | `Double` | - | Kullanıcı boylamı |
| `radiusMeters` | `Int` | `250` | Tarama yarıçapı (metre) |
| `busType` | `Int` | `54` | Şehir kodu |

**Yarıçap Seçenekleri:** `50m`, `100m`, `250m`, `500m`, `750m`, `1000m`, `1500m`

**Yanıt Modeli:** `List<NearLine>` (JSON Array)

| Alan | Tip | Açıklama |
|------|-----|----------|
| `lineId` | `Int` | Hat ID'si |
| `typeValueId` | `Int` | Araç türü ID'si |
| `typeValueName` | `String` | Araç türü adı ("ÖZEL HALK OTOBÜSÜ") |
| `typeValueColor` | `String` | Renk kodu ("#d389e8") |
| `lineName` | `String` | Hat adı |
| `lineNumber` | `String` | Hat numarası |
| `nearestDistanceMeters` | `Double` | En yakın mesafe (metre) |
| `routes` | `List<NearLineRoute>` | Hat güzergahları |

**`NearLineRoute` Modeli:**

| Alan | Tip | Açıklama |
|------|-----|----------|
| `routeId` | `Int` | Güzergah ID'si |
| `routeName` | `String` | Güzergah adı |
| `startLocation` | `String` | Başlangıç durağı |
| `endLocation` | `String` | Bitiş durağı |
| `distanceMeters` | `Double` | Mesafe (metre) |

**Örnek İstek:**

```bash
curl -H "Origin: https://ulasim.sakarya.bel.tr" \
     "https://sbbpublicapi.sakarya.bel.tr/api/v1/Ulasim/nearlines?latitude=40.725672&longitude=30.46342&radiusMeters=250&busType=54"
```

**Özellikler:**
- Sonuçlar mesafeye göre otomatik sıralanır
- Hat detayına gidip döndüğünüzde sonuçlar korunur (ViewModel backstack'e bağlı)
- Konum cihazın harita konumundan alınır

---

### 11. 🧭 Nasıl Giderim API

Başlangıç ve varış noktası arasında toplu taşıma güzergah önerileri sunar. OpenTripPlanner (OTP) tabanlıdır.

**Endpoint:**

```http
GET https://sbbpublicapi.sakarya.bel.tr/api/v1/Ulasim/otp/plan?fromPlace={lat},{lng}&toPlace={lat},{lng}&date={date}&time={time}&mode=TRANSIT,WALK
```
...
| Parametre | Tip | Format | Açıklama |
|-----------|-----|--------|----------|
| `fromPlace` | `String` | `lat,lng` | Başlangıç koordinatı |
| `toPlace` | `String` | `lat,lng` | Varış koordinatı |
| `date` | `String` | `MM-DD-YYYY` | Tarih |
| `time` | `String` | `HH:mm` | Saat |
| `mode` | `String` | - | Ulaşım modu (TRANSIT,WALK) |
...
| `NearLine.kt` | `NearLine`, `NearLineResponse`, `NearLineRoute` | Yakınımdaki hatlar |
| `StationEstimate.kt` | `StationEstimate` | Durağa yaklaşan otobüs tahmini |
| `RouteModels.kt` | `Itinerary`, `Leg`, `Stop` | Nasıl Giderim güzergah sonuçları |
...
│       ├── DurakDetailViewModel.kt   # Durak detay ViewModel
│       ├── NearLinesScreen.kt        # Yakınımdaki hatlar
│       ├── NearLinesViewModel.kt     # Yakınımdaki hatlar ViewModel
│       ├── HowToGoScreen.kt          # Nasıl Giderim
│       ├── RouteResultScreen.kt      # Güzergah sonuçları
│       ├── RouteViewModel.kt         # Güzergah ViewModel
│       ├── HatSeferSaatleriScreen.kt # Tüm hatlar sefer saati
│       ├── AracSorguScreen.kt        # Araç sorgulama (plaka/kapı no)
│       ├── AracSorguViewModel.kt     # Araç sorgulama ViewModel
│       ├── AracDetayScreen.kt        # Araç detay ekranı
